...
"""
    for nutrient in nutrients:
        try:
            ...
            value = float(value[:-2]) # 단위 제거 및 숫자 변환
        except ValueError:
            continue # 두 개의 값으로 분리되지 않는 경우 건너뛰기
        except (IndexError, ValueError):
            # value가 올바르게 변환되지 않는 경우 건너뛰기
            continue

        if '열량' in name:
            scores['calories'] = 10 - min(value / 100, 10)
        elif '나트륨' in name:
            scores['sodium'] = 10 - min(value / 200, 10)
        elif '탄수화물' in name:
            scores['carbohydrate'] = 10 - min(value/ 100, 10)
        elif '당류' in name:
            scores['sugar'] = 10 - min(value / 50, 10)
        elif '지방' in name:
            scores['fat'] = 10 - min(value / 50, 10)
        elif '트랜스지방' in name:
            scores['trans_fat'] = 10 if value == 0 else 0
        elif '포화지방' in name:
            scores['saturated_fat'] = 10 - min(value / 10, 10)
        elif '콜레스테롤' in name:
            scores['cholesterol'] = 10 - min(value / 300, 10)
        elif '단백질' in name:
            scores['protein'] = min(value / 10, 10)
"""
...
class ProductViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        # 특정 제품의 영양 성분 등급 계싼
        nutrient_info = serializer.data.get('nutrient', '')
        total_score, grade = calculate_score(nutrient_info)
